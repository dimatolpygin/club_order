...
def main() -> None:
    ...
    # Подписчик: главное — продлить + моя подписка; нет «вступить».
    check("подписчик: есть «продлить»", "renew" in sub, True)
    check("подписчик: есть «моя подписка»", "mysub" in sub, True)
    check("подписчик: нет «вступить»", "join" not in sub, True)
    check("подписчик: «продлить» — первое (главное действие)", sub[0], "renew")
    ...
    top_all = {"join", "renew", "mysub", "events", "mytickets", "aboutmenu", "support"}
    ...
    # Подпись главного действия подписчика обновлена.
    check("подпись «продлить подписку»", menu.BUTTON_DEFS["renew"][0], "Продлить подписку")
