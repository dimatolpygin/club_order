...
def main() -> None:
    ...
    # ── Скидка новичка по реф-ссылке (фикс ₽) + бонусы (этап 17) ──────────────
    # Новичок без подписки: скидка 500 ₽ от 1000 → 500 (referral).
    check("рефералка: скидка новичка 500 ₽ → 500 (referral)",
          ev.best_ticket_price(1000, referral_amount=500),
          (Decimal("500.00"), "referral"))
    # Скидки не суммируются: новичок 500 ₽ vs промокод 60% (400) → берём промокод.
    check("рефералка: промокод −60% (400) выгоднее скидки новичка 500₽ → promo",
          ev.best_ticket_price(1000, referral_amount=500, promo_pct=60),
          (Decimal("400.00"), "promo"))
    # Скидка новичка 500₽ vs промокод 30% (700) → выгоднее новичок (500).
    check("рефералка: скидка новичка 500₽ выгоднее промокода −30% → referral",
          ev.best_ticket_price(1000, referral_amount=500, promo_pct=30),
          (Decimal("500.00"), "referral"))
    # Скидка больше цены не уводит в минус.
    check("рефералка: скидка 500₽ от 300 → 0 (не минус)",
          ev.best_ticket_price(300, referral_amount=500),
          (Decimal("0.00"), "referral"))

    # Потолок оплаты бонусами: не более 50% от цены, целые ₽, не больше кошелька.
    check("бонусы: 50% от 1000 при кошельке 1000 → 500",
          ev.bonus_cap(1000, 1000), 500)
    check("бонусы: кошелёк меньше 50% → весь кошелёк (300)",
          ev.bonus_cap(1000, 300), 300)
    check("бонусы: 50% округляется вниз (от 999 → 499)",
          ev.bonus_cap(999, 100000), 499)
    check("бонусы: пустой кошелёк → 0",
          ev.bonus_cap(1000, 0), 0)
    check("бонусы: цена 0 → 0",
          ev.bonus_cap(0, 1000), 0)
