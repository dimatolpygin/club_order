...
def _parse_price(raw: str | None) -> Decimal | None:
    """Цена из строки. Пусто → None; некорректная/≤0 → ValueError.

    Терпим разделитель тысяч (пробел/неразрывный пробел — так поле показывает
    fmt_price, напр. «4 444») и запятую как десятичный разделитель.
    """
    # Убираем любые пробельные символы (обычный/неразрывный/узкий пробел, таб) —
    # это разделитель тысяч: поле показывает цену через fmt_price («4 444»).
